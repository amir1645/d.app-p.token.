...
function updateMinerUI(user, minerTokenPool) {
    ...
    // موجودی استخر ماینر برای توزیع (همان توکن P - بدون تبدیل)
    const minerTokens = ethers.utils.formatEther(minerTokenPool || '0');
    document.getElementById('miner-pool-balance').textContent = `${parseFloat(minerTokens).toFixed(2)} توکن P`;
    
    // تعداد توکن‌های موجود برای خرید (همان توکن P)
    document.getElementById('miner-tokens-available').textContent = `${parseFloat(minerTokens).toFixed(2)} توکن P`;
}
...
async function updateWithdrawInfo() {
    ...
    try {
        const [user, poolBalance, specialPoolBalance, specialPoolCount, totalUsers] = await Promise.all([
            ...
            contract.getSpecialPoolCount(), // مورد 23: تعداد تعادل‌ها
        ...
        document.getElementById('pool-balance-count').textContent = user.balanceCount.toString(); // مورد 14: تعادل‌های کاربر
