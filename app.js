...
function updateUserUI(user) {
    ...
    // وضعیت ماینر و پاداش‌ها
    document.getElementById('miner-status').textContent = user.isMiner ? 'فعال' : 'غیرفعال';
...
async function updateWithdrawInfo() {
    ...
    try {
        const [user, poolBalance, specialPoolBalance, specialPoolCount, totalUsers] = await Promise.all([
            ...
            contract.getSpecialPoolCount(), // مورد 23: تعداد سقف‌های ویژه
        ...
        document.getElementById('user-special-balance').textContent = user.specialBalanceCount.toString(); // مورد 14: موجودی ویژه کاربر
