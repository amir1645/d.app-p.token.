...
async function updateMinerStats() {
    ...
    try {
        const [user, minerPoolBalance] = await Promise.all([
            contract.getUserInfo(userAccount),
            contract.minerTokenPool()
        ]);
        
        updateMinerUI(user, minerPoolBalance);
...
function updateMinerUI(user, minerPoolBalance) {
    // وضعیت ماینر
    ...
    // پاداش‌های کل ماینر (تبدیل به پالیگان)
    const totalMinerRewardsMatic = ethers.utils.formatEther(user.totalMinerRewards || '0');
    document.getElementById('miner-total-rewards').textContent = `${parseFloat(totalMinerRewardsMatic).toFixed(4)} پالیگان`;
    
    // موجودی استخر ماینر برای خرید
    const minerPoolBalanceMatic = ethers.utils.formatEther(minerPoolBalance || '0');
    document.getElementById('miner-pool-balance').textContent = `${parseFloat(minerPoolBalanceMatic).toFixed(4)} پالیگان`;
}

// تابع به‌روزرسانی موجودی قابل خرید ماینر
async function updateAvailableMinerBalance() {
    if (!contract) return;
    ...
    try {
        ...
        document.getElementById('available-miner-balance').textContent = `${parseFloat(minerPoolBalanceMatic).toFixed(4)} پالیگان`;
        
    } catch (err) {
        console.error('Error updating available miner balance:', err);
    }
}
...
async function updateWithdrawInfo() {
    ...
    try {
        ...
        // استخر پاداش ویژه - بررسی مقدار و نمایش صحیح
        ...
        // دیباگ اطلاعات
        console.log('Special Pool Info:', {
            balance: specialPoolBalanceMatic,
            count: specialPoolCountValue,
            userSpecialBalance: user.specialBalanceCount.toString()
        });
        
    } catch (err) {
        console.error('Error updating withdraw info:', err);
        // نمایش خطا در کنسول برای دیباگ
        console.error('Detailed error:', err);
    }
}

// تابع به‌روزرسانی وضعیت استخر ویژه
async function updateSpecialPoolStatus() {
    ...
    try {
        ...
        const [specialPoolBalance, specialPoolCount, eligibleSpecialCount] = await Promise.all([
            contract.specialRewardPool(),
            contract.getSpecialPoolCount(),
            contract.eligibleSpecialUserCount().catch(() => '0')
        ]);
        ...
        if (balanceMatic === 0) {
            ...
            statusElement.innerHTML = '<i class="fas fa-exclamation-triangle"></i><span>استخر ویژه در حال حاضر خالی است</span>';
        } else if (eligibleUsers === 0) {
            statusElement.className = 'distribution-status warning';
            statusElement.innerHTML = '<i class="fas fa-exclamation-triangle"></i><span>هیچ کاربر واجد شرایطی برای برداشت وجود ندارد</span>';
        } else {
            statusElement.className = 'distribution-status info';
            statusElement.innerHTML = `<i class="fas fa-info-circle"></i><span>${eligibleUsers} کاربر واجد شرایط برای برداشت</span>`;
        }
